...
try:
    ...
    pyaudio = None
    print("[ASR] 警告: pyaudio 未安装 (pip install pyaudio)")

try:
    import websockets
except ImportError:
    websockets = None
    print("[ASR] 警告: websockets 未安装 (pip install websockets)")
...
class MicrophoneASR:
    ...
    async def _recognize(self, stream, chunk_size, first_chunk) -> str:
        headers = {
        ...
        try:
            async with websockets.connect(
                ...
                # 建连
                await ws.send(self._full_req(seq)); seq += 1
                await asyncio.wait_for(ws.recv(), timeout=5)
                # 发第一段
                await ws.send(self._audio_req(seq, first_chunk, False)); seq += 1

                silence_start = None
                texts = []
                while self._running:
                    try: data = stream.read(chunk_size, exception_on_overflow=False)
                    except: break
                    ...
                    if vol < settings.MIC_SILENCE_THRESHOLD:
                        ...
                        silence_start = None
                    await ws.send(self._audio_req(seq, data, False)); seq += 1
                    try:
    ...
    def _audio_req(self, seq, audio, is_last):
        h = bytearray([0x11])
        if is_last: h.append(0x23); seq = -seq
        else: h.append(0x21)
        h.extend([0x01, 0x00])
    ...
    def _parse(self, msg):
        try:
            if len(msg) < 4: return ""
            hs = msg[0] & 0x0f; mt = msg[1] >> 4; fl = msg[1] & 0x0f
            sr = msg[2] >> 4; cp = msg[2] & 0x0f
            p = msg[hs*4:]
            if fl & 0x01: p = p[4:]
            if fl & 0x04: p = p[4:]
            if mt == 0x09: p = p[4:]
            if not p: return ""
            if cp == 1: p = gzip.decompress(p)
            if sr == 1: return json.loads(p.decode()).get("result",{}).get("text","")
        except: pass
        return ""

    def pause(self): self._is_listening = False
    def resume(self): self._is_listening = True
    def on_text(self, cb): self._on_text_callbacks.append(cb)
    def _notify(self, t):
        for cb in self._on_text_callbacks:
            try: cb(t)
            except Exception as e: print(f"[ASR] 回调错误: {e}")
    def stop(self): self._running = False; print("[ASR] 已停止")