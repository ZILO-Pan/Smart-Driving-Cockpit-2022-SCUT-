"""
语音合成模块 (TTS)
文字 -> 火山引擎 TTS WebSocket -> pygame.mixer 播放
"""
...
class Speaker:
    ...
    def _synthesize(self, text):
        ...
        try:
            return loop.run_until_complete(self._ws_synth(text))
        finally:
            loop.close()

    async def _ws_synth(self, text):
        reqid = str(uuid.uuid4())
        ws_headers = {"Authorization": "Bearer;" + settings.TTS_ACCESS_TOKEN}
        payload = {
            "app": {"appid": settings.TTS_APP_ID, "token": "access_token", "cluster": settings.TTS_CLUSTER},
            "user": {"uid": "cockpit"},
            "audio": {
                "voice_type": settings.TTS_VOICE_TYPE,
                "encoding": settings.TTS_ENCODING,
                "speed_ratio": 1.1, "volume_ratio": 1.0, "pitch_ratio": 1.0,
